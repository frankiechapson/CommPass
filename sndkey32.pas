...
const
  ...
  UNITNAME         = 'SendKeys';
var
  UsingParens, ShiftDown, ControlDown, AltDown, AltGrDown, FoundClose : Boolean;
  PosSpace       : Byte;
  I, L           : Integer;
  NumTimes, MKey : Word;
  KeyString      : String[20];
...
//  If (Wait) then sleep(1);
//    While (PeekMessage(KeyboardMsg,0,WM_KEYFIRST, WM_KEYLAST, PM_NOREMOVE)) do sleep(1);
//    begin
...
begin
  ...
  If (BitSet(Hi(MKey),VKKEYSCANALTGRON)) then SendKeyDown(VK_RMENU  ,1,False);
  SendKeyDown(Lo(MKey), NumTimes, GenDownMsg);
  If (BitSet(Hi(MKey),VKKEYSCANSHIFTON)) then SendKeyUp(VK_SHIFT);
  If (BitSet(Hi(MKey),VKKEYSCANCTRLON))  then SendKeyUp(VK_CONTROL);
  If (BitSet(Hi(MKey),VKKEYSCANALTON))   then SendKeyUp(VK_MENU);
  If (BitSet(Hi(MKey),VKKEYSCANALTGRON)) then SendKeyUp(VK_RMENU);
end;
...
begin
  If (not UsingParens) then begin
    ...
    If AltGrDown   then SendKeyUp(VK_RMENU);
    ShiftDown   := false;
    ControlDown := false;
    AltDown     := false;
    AltGrDown   := false;
